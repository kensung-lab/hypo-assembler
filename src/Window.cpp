...
// Pick the one with minimum mismatches; if draw, the one with minimum distance
void Window::select_consensus_from_draft(bool is_marked)
{
    ...
    info += (std::to_string(min_id));
    
}
// Mismatches as ACGT; INS as acgt; del as D preceded by len; matches as M preceded by length
...
bool Window::has_long_indel(std::string& cig){
    ...
    for (auto c: cig) {
        if (c=='A' || c=='C'|| c=='G'|| c=='T') {
            ...
            if (len_ins >= RELIABLE_DRAFT_STH || std::stoi(del_len)  >= RELIABLE_DRAFT_STH) {return true;}
            len_ins = 0;
            del_len = "0";
        }
        else if (c=='M') {
...
// NOTE: Multiple return points here
bool Window::select_most_frequent() {
     ...
     * Therefore, lengths of arms with the cigar should also be considered in identifying the correspondence between sequence and cigar.
     * 
    */
    ...
    for (UINT32 i = 0; i < _internal_arms.size(); ++i)
    {
        ...
        if (has_long_indel(_internal_cigs[i])) {continue;}
    ...
    */
    if (counts[max_count_id] < 2*CONSENSUS_CONSTANT_THRESHOLD) { // proper-pair has wt 2; so real support is count/2
        return false;
    }
    ...
    if (counts.size()>1) { // more than 1 sequence present
        ...
        INT64 remaining = std::accumulate(counts.begin(), counts.end(), 0);;
        for (auto i =0; i< rank.size();++i) {
            ...
            info += (std::to_string(counts[cig_ind])+"_");
            ...
            if (i+1<rank.size()) {
                ...
                bool cond1 = counts[rank[i+1]] < 2*CONSENSUS_CONSTANT_THRESHOLD; // proper-pair has wt 2; so real support is count/2
                // this one makes > th (0.8) of the remaining
                bool cond2 = ((double)counts[cig_ind] / (double)remaining) > CONSENSUS_SUPPORT_FRACTION; // roper-pair has wt 2; so real support is count/2
                // next one is less than th (40%) of this one
                bool cond3 = (double)counts[cig_ind]*MOST_FREQUENT_FRACTION1 >counts[rank[i+1]];
                // we trust draft only if read sets have more mismatches (draft can not be trusted with read-sets having more indels)
...
std::string Window::get_SW_pileup_consensus(const std::string& backbone, const std::vector<std::string>& internal, const std::vector<std::vector<UINT16>>& iw, const std::vector<std::string>& pre, const std::vector<std::vector<UINT16>>& pw, const std::vector<std::string>& suf, const std::vector<std::vector<UINT16>>& sw) {
    ...
    for (UINT i = 0; i < backbone.size();){
        ...
        if (is_super_low_ps && cov[i]==1) { // in case of super low cov p/s, trust the draft rather than not properly paired read
            consensus.push_back(backbone[i]);
            ++i;
            continue;
        }
        ...
        if (is_super_low_ps){max_base_ind = cNt4Table[backbone[i]];} // trust draft base in suoerlow
        // Find blank count
        UINT16 del_len = 0;
        UINT16 num_blank = 0;
        ...
        if (!choose_del) {
            ...
            auto cc = std::min(cov[i],cov[i+1]); //(i+1 works because last base is tail and should have no ins)
            if (num_ins>0 && 2*num_ins > cc) { // add insertion 
                if (is_super_low_ps && ins_seq.size()>= RELIABLE_DRAFT_STH ) {;}// Do not trust long indel here
                else {consensus.append(ins_seq);}
                //std::cout << "Inserted " << i << ":" << ins_seq << "  ";
            }                      
            ++i;
        }
    }
...
{    /////////////////////// PATCH /////////////////////////
    ...
    auto patch_it = [&](const std::string& temp_cons) -> std::string {
        ...
        if (should_use_soft) {
            for (auto p: _pends) {if (p+cMarkerLen > patche) {should_use_soft=false; break;}}
        }
        if (should_use_soft) {
            for (auto s: _sbegs) {if (s+cMarkerLen < patchb) {should_use_soft=false; break;}}
        }
        if (should_use_soft && patchb>cMarkerLen && patche<_draft.get_seq_size()+cMarkerLen) {
            update_cliprun_support();
            if (!num_units.empty()) {info += "_SOFT";}
        }
        std::string unit;
        ...
        if (run_len==(qe-qb+1) && (run_len%unit.size()==0) && unit.size()==patch_unitlen && !num_units.empty()) { // patch
            ...
            for (auto it=num_units.begin(); it !=num_units.end();++it) {
                if (it->second==x->second){
                    if (occ) {return final_cons;} //already an occ found; Not unique=>DO not patch
                    occ = true;
                }
            }
    ...
    if (_internal_arms.size()>0) {
        ...
        for (auto k=0; k<iclusters[max_supp_id].size(); ++k) {if (2*iwt[max_supp_id][k]< icov[max_supp_id][k]) {is_all_supp=false;break;}}
    ...
    if (is_sh_low() || !int_found) { // if low coverage window or good rep from internal was not found
        // Find prefix clusters
        if (_pre_arms.size()>0) {
           get_pre_clusters();
         }
        
        // Find suffix clusters
        if (_suf_arms.size()>0) {
            get_suf_clusters();
        }
...
void Window::generate_consensus_long()
{
    ...
    std::vector<UINT32> clus_ids(_internal_arms.size(),0); // index of the cluster to which an arm belongs
    ...
    // Sort in decreasing order of aligned span
    ...
    if (_internal_arms.size()>0) { 
        ...
        auto cluster= [&](UINT r) -> void {
            ...
            clus_ids[i] = cig_id;                    
            while (curr_centre>=cNumDigits[curr_digitind+1]) {++curr_digitind;}   
        };
        ...
        if (_long_del>0) { // this window has a long deletion; choosethe cluster  closest to length of long del
            ...
            // For each cluster, find the reads falling in longest 5 aligned segments 
            std::vector<UINT64> numalens(cluslens.size(),0); 
            const UINT top = std::min((int)_internal_arms.size(),5);    
            for (UINT r = 0; r < top; ++r)
            {
                UINT i = rank2[r];
                auto alen = _areflen[i];
                if (alen>0) {
                    ++numalens[clus_ids[i]];
                    // info+=(std::to_string(i)+":"+std::to_string(alen)+"-");
                }
            } 
            ...
            UINT cutoff = std::max(MIN_CLUSTER_STRENGTH,std::ceil(max_cnt*LONG_CLUS_SECOND_TH));
            for (UINT c=0; c<cluslens.size(); ++c) {
                if (counts[c] >= cutoff) { // consider this cluster
                    if (numalens[c] > numalens[max_clus_id]) {
                        max_clus_id = c;
                        info+="MINOR";
                    }
                }
            ...
            */
            ...
            chosen_cluslen = cluslens[max_clus_id];
            if(_num_empty > 0) {                
                if (_num_empty >= cutoff) { // consider this cluster
                    if (_empty_maxalen >= _areflen[rank2[top-1]]) {
                         set_consensus(""); // empty sequence is consensus
                         info+="EMPTY";
                        return;
                    }
            ...
            info += "CLUSTERS:";
            for(UINT c = 0; c < cluslens.size(); c++) info += std::to_string(counts[c]) + "/" + std::to_string(cluslens[c]) + "#";
            info += "###";
            info += "CHOSEN_CLUSTER:"+ std::to_string(max_clus_id);

            //info += "_"+std::to_string(num_added_arms) + "_"+ std::to_string(numalens[max_clus_id]);
        }        
    }
...
// Returns whether seq spans refe; Assumes SW has been called; refb < refe; refb is first base of run; refe is the last base of the run
    ...
    for(UINT32 i = 0; i < _minimap_sw_results[_engine_idx]->cigar.size(); i++) {        
        ...
        if((bam_cigar_type(get_op) & 3) == 3) { // consume query and reference
            while(get_oplen > 0) {
                ...
                if (current_reference_index==refe) {
                    //dbg+=seq[current_query_index];
                    qe=current_query_index;does_span=true;break;
                }
                //dbg+=seq[current_query_index];
                get_oplen--;
...
/** Returns whether seq spans refb; Assumes SW has been called; refb < refe; refb is first base of run; refe is the last base of the run
 * At beg: extra bases in query before beg are included; dels in query=> base after the del is the first
 * At end: extra bases after the end not included; dels at end=> base before the del is the last
 * */
