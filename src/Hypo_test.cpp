...
void Hypo::create_alignments(const bool is_sr, const Mode mode, const UINT32 batch_id) {
        ...
        if (cid >= initial_cid) {
            if (is_sr) {  // short reads     
                ...
                if (!((_lalignment_store[cid].back())->is_valid)) {
                    ...
                    tag_position = bam_aux_get(hts_align, "SA");
            
                    if(tag_position) { // SA tag is found: it has supplementary
                        supplementary_location[_lalignment_store[cid].back().name].push_back(std::make_tuple(cid, _lalignment_store[cid].size())); // store for later use
                    }
    ...
    for(auto & it : supplementary_location) {
        ...
        if(get_seq.size() > 0) {
            ...
            current_idx_left = 0; // start at 0, join 0 with 1 if possible, then with 2, etc.
            current_idx_right = 1;
            while(current_idx_right < it.second.size()) {
                ...
                _lalignment_store[std::get<0>(it.second[current_idx_right])][std::get<1>(it.second[current_idx_right])], get_seq)) { // we can join, continue to check if right can join
                    current_idx_right++;
