...
void Hypo::polish() {
    ...
    /* Write results */
    _monitor.start();
    std::ofstream ofile(_cFlags.output_filename_1);
    if (!ofile.is_open()) {
        fprintf(stderr, "[Hypo::Hypo] Error: File open error: Output File (%s) could not be opened!\n",_cFlags.output_filename_1.c_str());
        exit(1);
    }
    //std::ofstream bedfile(BEDFILE);
    for (UINT64 i = 0; i < _contigs.size(); ++i) {
       _contigs[i]->generate_first_output(ofile);
       //_contigs[i]->generate_inspect_file(bedfile);
    }
    ofile.close();
    
    std::ofstream ofile2(_cFlags.output_filename_2);
    if (!ofile2.is_open()) {
        fprintf(stderr, "[Hypo::Hypo] Error: File open error: Output File (%s) could not be opened!\n",_cFlags.output_filename_2.c_str());
        exit(1);
    }
    //std::ofstream bedfile(BEDFILE);
    for (UINT64 i = 0; i < _contigs.size(); ++i) {
       _contigs[i]->generate_second_output(ofile2);
       //_contigs[i]->generate_inspect_file(bedfile);
    }
    ofile2.close();
